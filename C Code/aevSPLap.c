...
void aevSPLap(int NumTOp, int numIC, int numIQ, double taxC, double taxE, const
              ...
              emxArray_int32_T *bestexperimento, emxArray_real_T *trace, char* schedule_path, char* results_path)
{
  emxArray_real_T *IQ;
  int numObsIQ;
  emxArray_int32_T *IC;
  ...
  for (g = 0; g < generations; g++) {
    ...
    for (j = 0; j < numIC; j++) {
      ...
      sch(NumTOp, iidx, NumRec, b_PCPrO, b_PME, b_PMA, b_PMAn, b_PS, b_PCPO,
          ...
          &varargin_5, &d1, &d2, &d3, &d4, g==generations-1, schedule_path, results_path);
      fitness->data[6 * j] = d0;
