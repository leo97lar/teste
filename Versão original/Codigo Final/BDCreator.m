...
CompOp = [];                % Complejidad de la operacion
TimeOp = [];                % Tiempo de operacion
...
OpXEsp = [];                                 % Operaciones por especialidad
for i=1:length(POPE)
    
    aux = ones(POPE(i)*TipoOp/100,1)*i;
    OpXEsp = [OpXEsp; aux];
...
for i=1:length(PTO)
    ...
    aux = ones(PTO(i)*TipoOp/100,1)*Hr(i);
    TimeOp = [TimeOp; aux];
...
for i=1:length(PE)
    ...
    aux = ones(PPE(i)*NumTOp/100,1)*PE(i);
    EP = [EP; aux];
...
CExD = [INI;aux];            % Cantidad de entradas por dia
flag=1;
