...
from config import saves_folder, port, main_save, weekdays
import pandas as pd
...
@app.route('/comparar')
def render_comparar(methods=['GET', 'POST']):
    html_file = 'comparar.html'
    if request.method == 'GET':
        return render_template(html_file, save1='', save2='')
    save1 = request.form.get('save1', False)
    save2 = request.form.get('save2', False)
    if not (save1 and save2):
        return render_template(html_file, save1='', save2='')
    else:
        return render_template(html_file, save1=save1, save2=save2)
...
def render_teste(folder=None, methods=['GET', 'POST']):
    global last_opened
    ...
    if request.method == 'GET':
        ...
        if 'open' in request.form:
            ...
            return redirect('/run_model/' + new_save + '/teste/' + new_save)
