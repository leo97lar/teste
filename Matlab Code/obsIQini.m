...
for g=1:length(ProbXEst)
  Lim(g) = numGen*sum(ProbXEst(1:g),1)/100;
end

List = zeros(numGen,numObsIQ);
newIC = int32(repmat(List,1,numIQ));
    ...
    for u=1:numObsIQ
        ...
        for i=1:numGen
           ...
           for j=1:length(Qtem2)
               ...
               if sum(Qtem2(1:j),2)<=r && sum(Qtem2(1:j+1),2)>r
                   Obs(i,idx(j+1))=1;
                   Qtem = Qtem.*(1./(1-Qtem(:,idx(j+1))));
                   Qtem(:,idx(j+1))=0;
                   break
               elseif sum(Qtem2(1:j),2)>r
                   Obs(i,idx(j))=1;
